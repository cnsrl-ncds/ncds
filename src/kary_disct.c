...
//
//yk modified 1w prediction with discount
//
int
main(int n_arg, char *arg_value[]) {
	...
	*work_interval = 0;
	...
	for( file_counter = 0; file_counter < trace_number; file_counter++ ){
		...
		printf("Trace file:%s\n",trace_name);
		printf("1WWW\n");
		printf("Hash:%d, K_entry:%d, Threshold:%f, Interval:%d, Windows:%d, Discount=%f\n",HASH, KEY, T, INTERVAL, WINDOWS, DISCOUNT);
		...
		if( trace_set == 1 ) {
				...
				|| ( pcap_pkt_header.ts_sec > interval_time ) ){	
					...
					if( *interval >= (WINDOWS_2+3) ) {
						...
						FPRINT_variance_list( top_variance_2, "TOP_Variance_2W",\
										 (interval_time-INTERVAL*2), interval_time_usec, (*interval-1), threshold_2 * DISCOUNT );
					...
					if( *interval >= (WINDOWS_2+2) ) {
						...
						#ifdef Debug
							#if Debug >= 1
								printf("threshold_2 = %lf, discount=%f\n", threshold_2, DISCOUNT);
							#endif
						#endif
					}
					...
					if( (*interval >= 3) && ((*interval)%2 == 1) ) {
					 MA1(sketch_table, forecast_table_MA1);//type confert from int to double for sketch table with MA=1
						...
						#ifdef Debug
							#if Debug >= 1
								printf("threshold_3 = %lf, discount=%f\n", threshold_3, DISCOUNT );
							#endif
						#endif						
						top_variance_3_1 = find_variance( key_list, error_table_3, a, b, c, d, KEY, threshold_3  * DISCOUNT);
						FPRINT_variance_list( top_variance_3_1, "TOP_Variance_1W",\
										 (interval_time-INTERVAL*2), interval_time_usec, (*interval-1), threshold_3  * DISCOUNT);
					...
					MA1(sketch_table, forecast_table_MA1);//type confert from int to double for sketch table with MA=1
						...
						threshold_3 = find_threshold( error_table_3 );
							...
							#endif
						#endif						
						top_variance_3_2 = find_variance( key_list, error_table_3, a, b, c, d, KEY, threshold_3 * DISCOUNT );
			...
			if( *interval >= (WINDOWS_2+3) ) {
				top_variance_2 = find_variance( key_list, error_table_2, a, b, c, d, KEY, threshold_2 * DISCOUNT );
				FPRINT_variance_list( top_variance_2, "TOP_Variance_2W",\
								 (interval_time-INTERVAL*2), interval_time_usec, (*interval-1), threshold_2  * DISCOUNT);
